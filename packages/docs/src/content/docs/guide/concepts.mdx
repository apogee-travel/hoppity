...
Middleware is for **cross-cutting concerns only** — things that apply across the service but aren't specific to any one handler or contract. Logging, observability, custom topology augmentation.
...
| After    | Custom middleware  | Your own cross-cutting concerns                                         |

`withCustomLogger` should always go first so the custom logger is available to all subsequent middleware and to the framework itself during handler wiring and broker creation.
